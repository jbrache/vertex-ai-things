...
def get_location_coords(
    ...
    for i in location_args:
        params += location_args[i]
    params = params.strip()
    # Replace All space to %20
    params = params.replace(' ', "%20")
    print(params)
    url = f"https://maps.googleapis.com/maps/api/geocode/json?address={params}&key={GOOGLE_MAPS_API_KEY}"
...
st.markdown(
    ...
    Here we used the [Geocoding API](https://developers.google.com/maps/documentation/geocoding/overview) from Google Maps to geocode an address to make it easy to use and learn in this example.
    """
)
...
with st.sidebar:
    ...
    if ("GOOGLE_MAPS_API_KEY" in os.environ):
        GOOGLE_MAPS_API_KEY = os.environ.get("GOOGLE_MAPS_API_KEY")
        maps_api_key = str(GOOGLE_MAPS_API_KEY)
    else:
        maps_api_key = st.text_input("Google Maps API Key", 
                                     key="maps_api_key",
                                     type="password",
                                     help="Enter your Google Maps API key in the sidebar. You can get a key [here](https://developers.google.com/maps/documentation/geocoding/get-api-key).")
        GOOGLE_MAPS_API_KEY = maps_api_key
    
    if not maps_api_key:
        st.warning(
            "Enter your Google Maps API key in the sidebar. You can get a key [here](https://developers.google.com/maps/documentation/geocoding/get-api-key)."
        )
